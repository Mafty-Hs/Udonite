...
@Component({
  selector:    'game-table-template',
  template: `<div></div>`
})
export class GameTableComponentTemplate implements OnInit, OnDestroy, AfterViewInit  {
  rootElementRef: ElementRef<HTMLElement>;
  gameTable: ElementRef<HTMLElement>;
  gameObjects: ElementRef<HTMLElement>;
  ...
  protected isTransformMode: boolean = false;
  ...
  mouseGesture: TableMouseGesture = null;
  protected touchGesture: TableTouchGesture = null;
  ...
  protected _rightRotate(rotate: number, just: boolean=false): number {
    let tmp = rotate % 360;
    if (!just) {
      if (tmp > 180) {
        tmp = tmp - 360;
  ...
  protected setGameTableGrid(width: number, height: number, gridSize: number = 50, gridType: GridType = GridType.SQUARE, gridColor: string = '#000000e6') {
  ...
  protected removeSelectionRanges() {
