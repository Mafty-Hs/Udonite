...
export class CardStackListComponent implements OnInit, OnDestroy {
  ...
  drawCard(card: Card) {
    ...
    if (card.isFront) {
      this.chatMessageService.sendOperationLog(`${this.cardStack.name} から ${card.name} を取り出した`,"card");
    } else {
      this.chatMessageService.sendOperationLog(`${this.cardStack.name} から 1枚取り出して伏せた`,"card");
    }
  } 
